...
public class GetProviderByTypeHeadAction extends ActionSupport
{
   ...
   try{
	   ...
	   if (flag!=null && flag.equalsIgnoreCase("filter")) {
		...
		if (con != null)
		{
			...
			{
				providers = (Collection<Provider>)providerManager.filter(typehead);
			}
	
			else if (con.equalsIgnoreCase("All"))
			{
				providers = (Collection<Provider>)providerManager.filterAll(typehead);
			}
			else
			{
				providers = (Collection<Provider>)providerManager.filterByCon(con, typehead);
			}
		}
		else
		{
