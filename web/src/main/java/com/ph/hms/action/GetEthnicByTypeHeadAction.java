...
public class GetEthnicByTypeHeadAction extends ActionSupport
{
   ...
   try{
	   ...
	   if (flag!=null && flag.equalsIgnoreCase("filter")) {
		...
		if (con != null)
		{
			...
			{
				ethnics = (Collection<Ethnic>)ethnicManager.filter(typehead);
			}
	
			else if (con.equalsIgnoreCase("All"))
			{
				ethnics = (Collection<Ethnic>)ethnicManager.filterAll(typehead);
			}
			else
			{
				ethnics = (Collection<Ethnic>)ethnicManager.filterByCon(con, typehead);
			}
		}
		else
		{
