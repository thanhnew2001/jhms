...
public class GetWarehouseByTypeHeadAction extends ActionSupport
{
   ...
   try{
	   ...
	   if (flag!=null && flag.equalsIgnoreCase("filter")) {
		...
		if (con != null)
		{
			...
			{
				warehouses = (Collection<Warehouse>)warehouseManager.filter(typehead);
			}
	
			else if (con.equalsIgnoreCase("All"))
			{
				warehouses = (Collection<Warehouse>)warehouseManager.filterAll(typehead);
			}
			else
			{
				warehouses = (Collection<Warehouse>)warehouseManager.filterByCon(con, typehead);
			}
		}
		else
		{
