...
public class GetPaymenttypeByTypeHeadAction extends ActionSupport
{
   ...
   try{
	   ...
	   if (flag!=null && flag.equalsIgnoreCase("filter")) {
		...
		if (con != null)
		{
			...
			{
				paymenttypes = (Collection<Paymenttype>)paymenttypeManager.filter(typehead);
			}
	
			else if (con.equalsIgnoreCase("All"))
			{
				paymenttypes = (Collection<Paymenttype>)paymenttypeManager.filterAll(typehead);
			}
			else
			{
				paymenttypes = (Collection<Paymenttype>)paymenttypeManager.filterByCon(con, typehead);
			}
		}
		else
		{
