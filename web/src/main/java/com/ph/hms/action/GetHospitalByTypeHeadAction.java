...
public class GetHospitalByTypeHeadAction extends ActionSupport
{
   ...
   try{
	   ...
	   if (flag!=null && flag.equalsIgnoreCase("filter")) {
		...
		if (con != null)
		{
			...
			{
				hospitals = (Collection<Hospital>)hospitalManager.filter(typehead);
			}
	
			else if (con.equalsIgnoreCase("All"))
			{
				hospitals = (Collection<Hospital>)hospitalManager.filterAll(typehead);
			}
			else
			{
				hospitals = (Collection<Hospital>)hospitalManager.filterByCon(con, typehead);
			}
		}
		else
		{
